...
func main() {
	...
	Mode to operate in. "reader" supports get/list & check. "writer" supports create/update/delete
	`)
	...
	f.StringVar(
		&cfg.ExchangeURL, "exchangeurl", cfg.ExchangeURL, "scheme://host:port[/path] endpoint for the token exchange. Only required for mode=reader",
	)
	f.StringVar(
		&cfg.ClientID, "clientid", cfg.ClientID, "relying party client id to present to the token exchange. Only required for mode=reader",
	)
	f.StringVar(
		&cfg.ClientSecret, "clientsecret", cfg.ClientSecret, "relying party client secret to present to the token exchange. Only required for mode=reader",
	)
	...
	if cfg.Mode != "reader" && cfg.Mode != "writer" {
		exitOnErr(fmt.Errorf(
